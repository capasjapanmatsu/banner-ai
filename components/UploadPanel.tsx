...
import type { BackgroundEffect, ImageEffects } from "@/types/banner";
...
interface UploadPanelProps {
  ...
  onImageEffectChange?: (type: "reference" | "product" | "logo", effect: ImageEffects | null) => void;
}
      ...
      {mode === "banner" && (
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            {/* 参考バナーの画像エフェクト */}
            {getFileByType("reference") && onImageEffectChange && (
              <ImageEffectPanel
                onEffectChange={(effect) => onImageEffectChange("reference", effect)}
                previewImage={getFileByType("reference")?.url}
              />
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {/* 商品写真の画像エフェクト */}
            {getFileByType("product") && onImageEffectChange && (
              <ImageEffectPanel
                onEffectChange={(effect) => onImageEffectChange("product", effect)}
                previewImage={getFileByType("product")?.url}
              />
            )}
          </div>
